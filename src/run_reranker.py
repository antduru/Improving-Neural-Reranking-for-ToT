...
def rerank_with_cross_encoder(
    ...
    for query_id, ranked_doc_ids in tqdm(
        bm25_rankings.items(), desc="Reranking queries"
    ):
...
def main():
    ...
    result_path = (
        OUTPUT_DIR / f"reranker_{safe_model_name}_topk_{args.rerank_top_k}.json"
    )
    rankings_path = (
        OUTPUT_DIR
        / f"reranker_rankings_{safe_model_name}_topk_{args.rerank_top_k}.json"
    )
